...
"""
import json, time, requests, logging, copy
from bilibili_api import user, Verify, utils
...
class Login:
    ...
    def login_QR(self):
        ...
        uinfo = user.get_self_info(verify)
        logger.info(f"[登录成功] 昵称：{uinfo['uname']}  UID:{uinfo['mid']}")
        return verify
